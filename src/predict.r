...
data_dmatrix <- xgb.DMatrix(data = as.matrix(df))

# Load the trained model
model <- readRDS(PREDICTOR_FILE_PATH)

if (model_category == 'binary_classification') {
    raw_predictions <- predict(model, newdata = data_dmatrix)
    predictions_df <- data.frame(Prediction0 = 1 - raw_predictions, Prediction1 = raw_predictions)
} else if (model_category == "multiclass_classification") {
    raw_predictions <- matrix(predict(model, newdata = data_dmatrix), ncol = length(unique(target_classes)), byrow = TRUE)
    predictions_df <- as.data.frame(raw_predictions)    
}
colnames(predictions_df) <- target_classes
